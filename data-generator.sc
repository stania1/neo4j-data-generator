...
def writeCsv(filename: String)(rows: List[List[String]]) = {
  val f = new File(filename)
  val writer = CSVWriter.open(f)
  rows.foreach { writer.writeRow(_) }
}

def writeNodes(nodeCount: Int, numberOfProperties: Int, propertyKeyValueLength: Int, flush: List[List[String]] => Unit = writeCsv("nodes.csv") _): Unit = {
...
def writeRelationships(nodeCount: Int, relationshipsPerNode: Int, numberOfProperties: Int, propertyKeyValueLength: Int, flush: List[List[String]] => Unit = writeCsv("relationships.csv") _): Unit = {
